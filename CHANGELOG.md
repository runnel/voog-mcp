...
- **Phase 3 admin wrappers (S5, S6, S12, S13, MD5, MD6, N5):**
  ...
  - `element_move(site, element_id, position?, parent_id?)` MCP tool (added to existing `voog.mcp.tools.elements`) + matching CLI subcommand `voog element-move`. `PUT /elements/{id}/move`. Body is FLAT; at least one of position/parent_id required. Idempotent. **Scope note (N5):** operates on element INSTANCES, not `element_definitions` (schema) — different resource. element_definitions mutations remain passthrough.
