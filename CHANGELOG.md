...
### Added
- `site_snapshot` accepts an optional `force` boolean (default false). When omitted/false, preserves the v1.2.x "refuse existing directory" contract — typical interactive use. When `force=true`, allows writing into an existing directory; intended for automation/cron use cases that re-snapshot the same path on a schedule. Files from a prior snapshot may persist alongside new files if the underlying Voog state has shrunk between runs (e.g. a deleted product); callers wanting a clean overwrite should `rm -rf` the directory externally before invoking. Closes audit I15.
