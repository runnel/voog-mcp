...
- **Phase 6a (defensive security layer):**
  ...
  - ``_redact_query_string(qs)`` helper + ``_QUERY_STRING_VALUE_CAP=50`` in ``voog.client`` — applied to the DEBUG log line in ``_request`` (outgoing URL unchanged). Long query-string values (>50 chars, e.g. X-Amz-Signature presigned-URL signatures, or accidentally-leaked ``?api_key=…`` style tokens) are replaced with ``"k=***N-chars***"``; keys retained verbatim for debugging context. Closes audit S-9.
